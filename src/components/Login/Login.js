        ...
        <div className="col-md-4">
          <div className={classes.parent}>
            <Card className={classes.login}>
              <img src={imagenperfil} alt="" className={classes.profile} />
              <form onSubmit={functAutenticacion}>
                <input type="text" placeholder="Ingresar Email" className={classes.input} id="email" />
                {registrando && (
                  <input type="text" placeholder="Ingresar Nombre de Usuario" className={classes.input} id="username" />
                )}
                <input type="password" placeholder="Ingresar Contraseña" className={classes.input} id="password" />
                <Button type="submit">{registrando ? "Registrate" : "Inicia Sesión"}</Button>
              </form>
              <footer className={classes.alternative}>
                <h4 className={classes.message}>{registrando ? "Si ya tienes una cuenta" : "No tienes cuenta"}{" "}
                </h4>
                <Button color={"info"} onClick={() => setRegistrando(!registrando)}>
